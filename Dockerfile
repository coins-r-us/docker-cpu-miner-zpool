...
RUN apt-get update && apt-get -y --no-install-recommends install git wget psmisc bash build-essential libssl-dev zlib1g-dev libgmp-dev libcurl4-openssl-dev libjansson-dev libpthread-stubs0-dev automake &&  apt-get install -y --reinstall ca-certificates && rm -rf /var/lib/apt/lists/* && apt-get clean all && (mkdir -p /usr/local/share/ca-certificates/cacert.org || true) &&  wget -P /usr/local/share/ca-certificates/cacert.org http://www.cacert.org/certs/root.crt http://www.cacert.org/certs/class3.crt && update-ca-certificates
